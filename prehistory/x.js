...
y='gggggggggg'
bstring=y+y+"g468ac864gg22222222g"+x+x+x+x+"g33333333gg579bd975g"+y+y  //in base 26 (g is 16)
wstring=x+x+x+"000111100000123321000123553210"
m=[0,0,[1,10],[21,19,12,8],[11,9],[1,10,11,9],[1,10,9,11],0]
moves=[]
//pawn=[8,48]
...
kx=[5,5]
kp=[25,95]
pv=[0,0,1,1,5,5,3,3,3,3,9,9,63,63,0,0]
pn=[]
weight=[]
...
for(z=0;z<16;){
    pv[z]=10*pv[z]  //same piece values for black and white
    if(m[z>>1]){
        s=m[z].length           //probably some better way
        moves[z]=m[z]
        for(x=0;x<s;){
            moves[z][s+x]=-m[z][x++]
        }
...
// bm== blackness flag
// k
// yx,h,aa,a,cx,mv,k=-1,bm=bm>>3,dir=bm*10-20,mvl=[],mvlength,wate

function parse(bm,EP,tpn){
//	var bord=board
    var tyx,yx,h,aa,a,cx,mv,k=-1,nbm=1-bm,dir=10-bm*20,mvl=[],m,wate,cbm=castle[bm]
    for(yx=21;yx<99;yx++){
		a=board[yx]
        if(a&15&&bm==(a&1)){
            wate=weight[yx]*M2
            a&=14
            if(a>2){    //non-pawns
            	ic=(closeness[a][40+kp[nbm]-yx]||0)//initial closeness
                ak=a==12
                mv=moves[a]
                if(a==6||ak){
                   for(cx=0;cx<8;){     //knights,kings
                        ...
                        aa=board[tyx]
                        if(!aa||(aa&17)==nbm){
                            mvl[++k]=[tpn+pv[aa]+(ak?0:weight[tyx]*M1)-wate+(ak&&moveno<30?(closeness[a][40+kp[nbm]-tyx]||0)*M3-ic:0),yx,tyx] //rating,start,end,-- enpassant left undefined
                        }
                    }
                    if(ak&&cbm){
                        if(cbm&1&&!(board[yx-1]+board[yx-2]+board[yx-3])&&check(yx-2,nbm,dir,-1)){//Q side
                            mvl[++k]=[tpn+11,yx,yx-2]                                //no analysis, just encouragement
                        }
                        if(cbm&2&&!(board[yx+1]+board[yx+2])&&check(yx,nbm,dir,1)){//K side
                    ...
                    for(cx=0;cx<mlen;){     //goeth thru list of moves
                        ...
                        while(!aa){   //while on board && no piece
                            ...
                            aa=board[tyx]
                            if(!aa||(aa&17)==nbm){
                                mvl[++k]=[tpn+pv[aa]+(weight[tyx]*M1)-wate+(closeness[a][40+kp[nbm]-tyx]||0)*M3-ic,yx,tyx]
                                //if ((aa&7)==6)return 0
                            }
                        }
                    }
                ...
                if(EP&&(EP==tyx+1||EP==tyx-1)&& bm!=(board[EP-dir]&1)) {       //enpassant
...
function check(yx,nbm,dir,side){         //dir is dir
    ...
    for(;yx<ex;yx++){ //go thru 3positions, checking for check in each
        for(m=dir-2;++m<md;){
			aa=board[yx+m]
            if(aa&&(aa&1)==nbm&&((aa&14)==2||(aa&14)==12))return 0        //don't need to check for pawn position --cannot arrive at centre without passing thru check
            aa=0
            tyx=yx
            while(!aa){   //while on board && no piece
                tyx+=m
                aa=board[tyx]
                if (aa&16)break
                if((aa==nbm+2+(m==dir)*2)||aa==nbm+10)return 0
            }
        }
        for (z=0;z<8;){
            if(board[yx+moves[3][z++]]-nbm==6)return 0      //knights
        }
    }
    aa=0
    yx-=3
	while (!aa){   //queen or rook out on other side
		yx-=side
		aa=board[yx]
		if(aa==nbm+4||aa==nbm+10)return 0
	}
	return 1
}
...
function move(s,e,queener,score){
    var a=board[s]&7,bm=bmove>>3,dir=10-bm*20,aa=board[e],x=s%10,tx=e%10,ty=e-tx,gap=e-s
    ep=0
    if(a==1){ //pawns
//     	debug(board[e+dir],"piece:",5-queener,'e',e,'s',s)
    ...
    if(s==21+bm*70||s==28+bm*70)castle[bm]&=(x<5)+1        //castle flags
    if(a==6){
